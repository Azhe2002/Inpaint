...
class InpaintDataset(Dataset):
    ...
    def __len__(self):  # 添加缺失的__len__方法
    ...
    def __getitem__(self, idx):
        ...
        # 合并为4通道输入
        input_tensor = torch.cat([img, mask_tensor], dim=0)
        return input_tensor, img  # 目标为完整图像
    ...
    try:
        ...
        for epoch in range(EPOCHS):
            ...
            for batch_idx, (inputs, targets) in enumerate(dataloader):
                ...
                loss = criterion(outputs, targets, inputs[:, 3:, :, :])  # 提取mask部分
                loss.backward()
                optimizer.step()
