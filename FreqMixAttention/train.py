...
def train():
    ...
    criterion = InpaintLoss(use_frequency_loss=True, use_color_consistency_loss=True, lambda_freq=0.1, lambda_color=0.1)
    ...
    try:
        ...
        for epoch in range(EPOCHS):
            ...
            for batch_idx, (images, masks, targets) in enumerate(dataloader):
                #print(images.shape, masks.shape)  # images: [Batch, 3, Height, Width], masks: [Batch, 1, Height, Width]
                images, masks, targets = images.to(device), masks.to(device), targets.to(device)

                # 合并输入（添加频域通道）
                inputs = add_frequency_channels(images, masks)
                #print(inputs.shape)  # 检查是否为 [Batch, 6, Height, Width]
    ...
    # 保存模型
    torch.save(model.state_dict(), "image_inpaint_6channel.pth")
    print("训练完成，模型已保存为 image_inpaint_6channel.pth")
