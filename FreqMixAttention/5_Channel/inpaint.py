...
def inpaint():
    ...
    mask_binary = np.where(mask_np > 128, 1, 0).astype(np.float32)  # 二值化
    mask_tensor = torch.from_numpy(mask_binary).unsqueeze(0).unsqueeze(0).float()  # 形状 [1, 1, H, W]

    # 转换图像为张量
    image_tensor = transform(image).unsqueeze(0)  # 形状 [1, 3, H, W]

    # 合并输入，添加频域通道
    model_input = add_frequency_channel(image_tensor, mask_tensor).to(device)  # 形状 [1, 5, H, W]
    ...
    # 后处理
    output_np = output.squeeze().cpu().numpy().transpose(1, 2, 0)  # 转换为 [H, W, C]
    output_np = (output_np * 0.5 + 0.5) * 255  # 反归一化到 [0, 255]
